...
namespace Inputs {
  ...
  // Timer-screen context (touch): when the bare RUNNING/PAUSED timer is showing,
  // StateMachine flags it so a tap on a transport circle dispatches directly
  // (instead of falling through to a generic select that opens the overlay).
  void setTimerScreenActive(bool on);
  int  timerAction();          // TA_NONE/TA_RESET/TA_PAUSE/TA_SKIP for this tick

  // Raw-tap mode (on-screen keyboard): when active, a tap latches its (x,y) here
  // and is NOT routed to menu/timer handling. Swipes still drive rotationDir().
  void setRawTouchActive(bool on);
  bool tapped();               // a tap occurred this tick (consume via tapX/tapY)
  int  tapX();
  int  tapY();
