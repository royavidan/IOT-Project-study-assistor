...
static void loadCreds()
{
  ...
  while (s_baseUrl.endsWith("/"))
    s_baseUrl.remove(s_baseUrl.length() - 1);
}
#endif
...
{
  if (s_baseUrl.length() == 0 || s_secret.length() == 0)
    return -1;
  if ((int32_t)(millis() - s_netSuspendUntil) < 0)
    return -99;
  if (!s_httpReuseInit)
  {
    s_http.setReuse(true);
    s_httpReuseInit = true;
  }
  ...
  if (https)
  {
    Serial.println("[cloud] https URL needs CLOUD_USE_TLS=1 in config.h");
    return -3;
  }
  ok = s_http.begin(s_plain, url);
  ...
  {
    s_netSuspendUntil = millis() + NET_FAIL_BACKOFF_MS;
    s_serverOk = false;
  }
...
static void jStr(const String &b, const char *key, char *out, size_t n)
{
  ...
  out[0] = 0;
  int p = valuePos(b, key);
  if (p < 0 || p >= (int)b.length() || b[p] != '"')
    return;
  p++;
  size_t i = 0;
  while (p < (int)b.length() && b[p] != '"' && i + 1 < n)
    out[i++] = b[p++];
  out[i] = 0;
}
...
static void patchOfflineTimestamps(String &json)
{
  if (time(nullptr) <= 1700000000L)
    return; // no clock yet — leave as-is
  static const char *PLACE = "1970-01-01T00:00:00Z";
  if (json.indexOf(PLACE) < 0)
    return; // already has real timestamps
...
static void drainUploadQueue()
{
  if (s_uploadAuthFail)
    return;
  if (!s_cachedOnline || UploadQueue::pendingCount() == 0)
    return;
  if (!s_uploadSoon && millis() - s_lastUploadTry < s_uploadBackoffMs)
    return;

  s_lastUploadTry = millis();
  String json;
  if (!UploadQueue::readOldest(json))
    return;                     // FS read (mutex'd internally)
...
static const char *stateStr(uint8_t st, uint8_t mode)
{
  ...
  {
  case ST_RUNNING:
    return mode == MODE_WORK ? "work" : "break";
  case ST_PAUSED:
    return "paused";
  default:
    return "idle";
...
static void pushTelemetry()
{
  ...
  String ts = (time(nullptr) > 1700000000L)
                  ? Payload::isoFromEpoch(time(nullptr))
                  : "1970-01-01T00:00:00Z";
  String body = "{";
  body += "\"deviceId\":\"" + s_deviceId + "\",";
  body += "\"ts\":\"" + ts + "\",";
  body += "\"state\":\"";
  body += stateStr(s.state, s.mode);
  body += "\",";
...
static void doPairing()
{
  ...
  if (c != 200)
    ...
    s_configNow = true; // poll soon in case the user claims immediately
}
...
static bool doUnpair()
{
  ...
  if (c != 200)
  {
    Serial.printf("[cloud] unpair failed (%d): %s\n", c, resp.c_str());
    return false;
  }
...
static void handlePortalRoot()
{
  ...
  // Render from the async-scan snapshot taken before the portal rose. Never
  // call the blocking WiFi.scanNetworks() here — this handler runs on the
...
static void handlePortalSave()
{
  ...
  String body = "<!DOCTYPE html><html><head><meta name=viewport content='width=device-width,initial-scale=1'>"
                ...
                "<h2>Saved &mdash; reconnecting&hellip;</h2><p>The MindBox is joining <b>" +
                ssid +
...
static void manageWifi()
{
  ...
  if (conn)
  { // snapshot SSID/IP here (core 0) so the UI getters never call WiFi.* on core 1
  ...
  if (!conn)
  {
    s_serverOk = false;
    s_lastHttpStatus = 0;
  } // no link -> backend definitely unreachable
  if (!conn)
  {
  ...
  else
  {
    if (!s_ntpStarted)
    {
      ...
      tzset(); // ...but make localtime_r() return local time (also fixes quiet hours)
...
static void cloudTask(void *)
{
  ...
  for (;;)
  {
    ...
    if (s_portalActive)
    {
      ...
      if (s_portalStopReq || timedOut || saveDone)
      {
        s_portalStopReq = false;
        stopPortal();
      }
    ...
    if (s_scanRunning)
    {
      ...
      if (r >= 0)
      {
        ...
        for (int i = 0; i < r && n < WIFI_SCAN_MAX; i++)
        {
          ...
          if (dup)
            continue;
          strncpy(s_scanList[n].ssid, ss.c_str(), 32);
          s_scanList[n].ssid[32] = 0;
        ...
        { // sort by signal, strongest first
          ScanItem t = s_scanList[a];
          int b = a - 1;
          while (b >= 0 && s_scanList[b].rssi < t.rssi)
          {
            s_scanList[b + 1] = s_scanList[b];
            b--;
          }
          s_scanList[b + 1] = t;
        }
    ...
    if (s_cachedOnline && millis() - s_firstOnlineMs > FIRST_NET_SETTLE_MS)
    {
      ...
      else if (s_unpairPending)
      {
        if (doUnpair())
          s_unpairPending = false;
      }
      ...
      else if ((s_pushNow && millis() - lastTele > TELEMETRY_MIN_GAP_MS) || millis() - lastTele > TELEMETRY_PERIOD_MS)
      {
        s_pushNow = false;
        lastTele = millis();
        pushTelemetry();
...
namespace Cloud
{
  ...
  {
#if ENABLE_CLOUD
    if (!s_settingsReady || !s_mux)
      return false;
    xSemaphoreTake(s_mux, portMAX_DELAY);
    out = s_settings;
    s_settingsReady = false;
    xSemaphoreGive(s_mux);
    return true;
#else
    (void)out;
  ...
  {
#if ENABLE_WIFI
    if (s_scanRunning || i < 0 || i >= s_scanN || !out || n == 0)
      return false;
    strncpy(out, s_scanList[i].ssid, n - 1);
    out[n - 1] = 0;
    rssi = s_scanList[i].rssi;
    secured = s_scanList[i].secured;
    return true;
#else
    (void)i;
    (void)out;
  ...
  bool uploadSession(const SessionRecord &r, const Sample *samples, int n)
  {
    ...
    if (!UploadQueue::enqueueJson(r.clientSeq, json))
      return false;
    s_uploadSoon = true;
    return true;
#else
  ...
  {
#if ENABLE_CLOUD
    if (!UploadQueue::enqueueJson(clientSeq, json))
      return false;
    s_uploadSoon = true;
    return true;
#else
  ...
  void provisionFromSerial()
  {
    ...
    if (ssid.length())
      Storage::setWifiSsid(ssid);
    if (pass.length())
      Storage::setWifiPass(pass); // blank = keep current (was unconditional — wiped the pw!)
    if (url.length())
      Storage::setAppBaseUrl(url);
    if (sec.length())
      Storage::setDeviceSecret(sec);
