...
| **homework/** | assignment cards + dialog (data lives in `lib/queries/homework.ts`) | `/assignments` |
...
Routes not owned by a feature: `/` (dashboard), `/settings`, `/login`, `/auth/callback`, `/reviewer/accept`.
...
- **Env access** (`lib/config.server.ts`): read `process.env` *inside* a function, never at module scope
