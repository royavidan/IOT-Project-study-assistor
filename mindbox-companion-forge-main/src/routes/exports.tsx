...
function Exports() {
  const navigate = useNavigate({ from: "/exports" });
  const { student: studentParam } = Route.useSearch();
  const { user, students, viewingStudent, isReviewerView, scopedSessions, sessionsQuery } =
  ...
  const onStudentChange = (value: string) => {
    if (value === "mine") {
      navigate({ search: { student: undefined }, replace: true });
    } else {
      navigate({ search: { student: value }, replace: true });
    }
  };
  ...
  return (
    <>
      <PageHeader
        title={isReviewerView ? "Export student report" : "Exports"}
        description={
          isReviewerView
            ? `Read-only export for ${viewingStudent?.displayName ?? "student"}.`
            : "Download or share your focus history."
        }
      />
