...
function toDbRow(userId: string, input: ScheduleEventInput) {
  ...
  return {
    ...
    category: input.category === "exam" ? "exam" : input.category === "study" ? "study" : "class",
    // Meeting subtype applies to classes only; exams + study blocks have none.
    subtype: input.category === "class" ? (input.subtype ?? null) : null,
    kind: input.kind,
